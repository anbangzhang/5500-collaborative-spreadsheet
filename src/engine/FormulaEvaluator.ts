...
export class FormulaEvaluator {
  ...
  getCellValue(token: TokenType): [number, string] {
    ...
    if (cell === undefined || cell.getFormula().length === 0) {
      return [0, ErrorMessages.invalidCell];
    }
    let evaluator = new FormulaEvaluator(this._sheetMemory);
    evaluator.evaluate(cell.getFormula());
    if (evaluator.error !== "") {
      return [0, evaluator.error];
    }
    return [evaluator._result, evaluator._errorMessage];
  }
